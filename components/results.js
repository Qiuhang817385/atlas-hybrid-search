  ...
  useEffect(() => {
    ...
    if (rerank && rerankedResults) {
      ...
      axios.post('api/rerank', { documents: response.results, query: queryText })
        ...
        .catch(error => {
          console.log(error);
          pushToast({ timeout: 10000, variant: "warning", title: t('toasts.apiFailure'), description: t('toasts.rerankingFailed', { error }) });
        });
    }
    ...
    <div>
      {
        results ?
          <div style={{ paddingLeft: "40px", paddingRight: "40px" }}>
            <div style={{ paddingTop: "25px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "20% 50% 30%", gap: "5px", alignItems: "start" }}>
                {(model?.reranking?.provider && rerankOpt) ? <div style={{ padding: "4px 16px" }}><Checkbox checked={rerank} bold={true} label={t('search.useReranker')} onChange={event => setRerank(!rerank)} /></div> : <></>}
                <div style={{ padding: "4px 16px" }}><span style={{ borderRadius: "5px", backgroundColor: palette.blue.light3, padding: "2px 4px", float: "left" }}><em>{msg ? `${msg}, ${t('search.queryTime', { time })}` : `${t('search.queryTime', { time })}`}</em></span></div>
                <div style={{ padding: "4px 16px" }}><Button style={{ float: "right" }} onClick={() => setOpen(true)} variant="default">{t('search.showQuery')}</Button></div>
              </div>
              {
                          ...
                          <div style={{ display: "grid", gridTemplateColumns: "60px 90%", gap: "5px", alignItems: "start" }}>
                            ...
                            <div>
                              ...
                              {hybrid ?
                                ...
                                : <p key={`${r._id}score`}>{t('search.score')} : <span style={{ fontWeight: "normal" }}>{r.score}</span></p>
                              }
                              {r.boost ? <Banner style={{ margin: "10px" }} variant="warning" image={<Bulb />}>{t('search.semanticallyBoosted')}</Banner> : <></>}
                              {r.reranked ? <Banner style={{ margin: "10px" }} variant="info">{t('search.reranked', { score: r.rerank_score })}</Banner> : <></>}
                            </div>
                          </div>
                        </Card>
                      </SearchResult>
            ...
            <Modal open={open} setOpen={setOpen}>
              <Subtitle>{t('modals.mongodbAggregationPipeline')}</Subtitle>
          ...
          :
          <div style={{ padding: "16px 16px" }}><Banner variant="warning">{noResultsMsg || t('search.noResults')}</Banner></div>
      }</div>
  )
}
